...
class Order(Strategy):
    ...
    def __init__(self, field, order = 'asc'):
        ...
        if isinstance(field, dict):
            ...
            self.conditions[field] = order
            
    def prepare(self, Q, pos = 0):
        result = ''
        if self.conditions is not None:
            for field,order in self.conditions.items():
                order = order.upper()
                result += ',{} {}'.format(field,order)
            result = result[1:] if pos == 0 else result
            result = "{} {}".format(" ORDER BY " if pos == 0 else " ",result)
            Q.query += result